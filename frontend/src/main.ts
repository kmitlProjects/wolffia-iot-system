...
import {
    ...
    CAMERA_RETRY_MS,
    DAY_OPTIONS,
    DEFAULT_FERTILIZER_WATER_LITERS,
    DEFAULT_WATER_PUMP_LITERS,
    EVERYDAY_VALUES,
    FALLBACK_TIMEZONE,
    LIVE_ANALYSIS_REFRESH_MS,
    LIVE_ANALYSIS_RETRY_MS,
    POLL_HIDDEN_MS,
    POLL_VISIBLE_MS,
} from "./constants.js?v=20260405ae"
import {
    formatCountdownLabel,
    formatCoverageMethod,
    formatCoverageProcess,
    formatFullDateLabel,
    formatInteger,
    formatNumber,
    formatRoiSize,
    ...
    formatTimeOnly,
    formatTimestamp,
} from "./formatters.js?v=20260405ae"
import { $, escapeHtml, renderIcon } from "./html.js?v=20260405ae"
import { createLayout } from "./layout.js?v=20260405ae"

// Runtime state shared across dashboard renderers and event handlers.
...
let anomalyLastCheckResult: AnomalyCheckResult | null = null
...
// Date/time helpers below still depend on dashboard state because timezone is runtime data.
...
// Generic data helpers used across multiple widgets.
...
// Timers and live-refresh controls
...
function formatAnomalySource(source: string | null | undefined): string {
...
function renderAnomalyMetricCard(
    label: string,
    value: number | null | undefined,
...
function renderAnomalyWatch(state: DashboardState): void {
    ...
    if (
        ...
        || !(debugCopy instanceof HTMLElement)
        || !(lastCopy instanceof HTMLElement)
    ...
    const currentLightState = String(
        status.last_light_state
        ?? lastCheck?.light_state
        ?? "",
    ).trim()
...
// App lifecycle and event binding
